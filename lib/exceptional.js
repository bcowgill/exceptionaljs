...
(function(exports, global) {
	...
	var ExtendedException = require('extended-exceptions');

	//dbg: var debug = require('debug')('exceptional');
	...
	function _extend (object, addons) {
	...
	var self = exports.exceptional = {
		...
		'augment': function (typeName) {
			...
			_methodNamesOf(methods).forEach(function (method) {
				...
				//dbg: debug('attach ', typeName, _super, _super in maker.target);
		...
		'unaugment': function (typeName) {
			...
			_methodNamesOf(methods).forEach(function (method) {
				...
				//dbg: debug('delete ', typeName, _super, _super in maker.target);
		...
		// throwIf, throwUnless, use extended-exceptions
		...
		'InvalidState': _extend(ExtendedException.create_custom_error(
			'InvalidState', ExtendedException.IllegalState), {
				throw: function (state, allowed, param, method) {
					allowed = allowed || [];
					method = method? (method + '() ') : '';
					param = param? (param + ' ') : '';
					var error = new self.InvalidState(method + param + 'state <' + state +
